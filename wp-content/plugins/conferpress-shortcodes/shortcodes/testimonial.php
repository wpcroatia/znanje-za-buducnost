<?php
function parse_sc_testimonial($atts, $content=''){
	//get parameter
...
add_shortcode( 'sc_testimonial', 'parse_sc_testimonial' );

function parse_sc_testimonial_item($atts, $content=''){
	$ID = isset($atts['ID']) ? $atts['ID'] : rand(10,9999);
...
add_shortcode( 'sc_testimonial_item', 'parse_sc_testimonial_item' );

//Visual Composer
add_action( 'after_setup_theme', 'reg_sc_testimonial' );
function reg_sc_testimonial(){
	if(function_exists('vc_map')){
		//parent
		vc_map( array(
			"name" => __("Testimonials", "leafcolor"),
			"base" => "sc_testimonial",
			"as_parent" => array('only' => 'sc_testimonial_item'), // Use only|except attributes to limit child shortcodes (separate multiple values with comma)
			...
			"params" => array(
				...
				array(
					...
					"description" => __('Auto scroll Testimonials','conferencepro')
				),
			),
		...
		vc_map( array(
			...
			"base" => "sc_testimonial_item",
			"content_element" => true,
			"as_child" => array('only' => 'sc_testimonial_item'), // Use only|except attributes to limit parent (separate multiple values with comma)
	...
	class WPBakeryShortCode_sc_testimonial extends WPBakeryShortCodesContainer{}
	class WPBakeryShortCode_sc_testimonial_item extends WPBakeryShortCode{}
	}
}